...
{  name = {js|Using postgres with OCaml|js} 
  ; description = {js|Huge session of troubleshooting, but that is how coding really is.
                 ppx_rapper= http=//opam.ocaml.org/packages/ppx_rapper/
                 examples-reason-postgres= https=//github.com/EduardoRFS/examples-reason-postgres/|js}
...
{  name = {js|State of the OCaml Platform 2020|js}
  ; description = {js|This talk covers=
...
{  name = {js|OCaml-CI = A Zero-Configuration CI|js}
  ...
  OCaml versions and OS platforms...|js} 
...
{ name= {js|The final pieces of the OCaml documentation puzzle|js} 
  ; description= {js|Rendering OCaml document is widely known as a very difficult task=
...
{ name= {js|API migration= compare transformed|js} 
  ; description= {js|In this talk we describe our experience in using an automatic API-migration
...
{  name= {js|Parallelising your OCaml Code with Multicore OCaml|js} 
  ; description= {js|Slides, speaker notes and runnable examples mentioned in this talk
...
{ name= {js|A Simple State-Machine Framework for Property-Based Testing in OCaml|js} 
  ; description= {js|Since their inception, state-machine frameworks have proven their
...
{  name= {js|The ImpFS filesystem|js} 
  ; description= {js|This proposal describes a presentation to be given at the OCaml\u201920
...
{  name= {js|Irmin v2|js}
  ; description= {js|Irmin is an OCaml library for building distributed databases with
...
{ name= {js|AD-OCaml= Algorithmic Differentiation for OCaml|js} 
  ; description={js|AD-OCaml is a library framework for calculating mathematically exact
...
{ name= {js|OCaml Under The Hood= SmartPy|js}  
  ; description= {js|SmartPy is a complete system to develop smart-contracts for the Tezos
...
{  name= {js|A Declarative Syntax Definition for OCaml|js}
  ; description= {js| In this talk, we present our work on a syntax definition for the OCaml
...
{ name= {js|LexiFi Runtime Types|js} 
  ; description= {js|OCaml programmers make deliberate use of abstract data types for composing
...
{  name= {js|Types in Amber|js} 
  ; description= {js|Coda is a new cryptocurrency that uses zk-SNARKs to dramatically reduce
